...
try:
    from junitparser import JUnitXml, TestSuite, TestCase, Failure, Skipped, Error  # type: ignore
except Exception as e:
    raise SystemExit("This script requires 'junitparser'. Install it with: pip install junitparser") from e 
def parse_files(paths: List[str]) -> Dict[str, Any]:
    suites_data = []
    case_rows = []

    def status_of(case: TestCase) -> str:
        # junitparser maps results as elements of case.result (list-like)
        if case.result:
    ...
    def detail_of(case: TestCase) -> str:
        if case.result:
            r = case.result[0]
...
def render_html(data: Dict[str, Any], title: str = "JUnit Report") -> str:
    t = data["totals"]
    ...
    def row(c: Dict[str, Any]) -> str:
        ...
        need_details = status in ("fail", "err") or (status=="skip" and (c["message"] or c["detail"]))
        ...
        if need_details:
          ...
          <div class="case-details">
            <div class="detail-head">
              ...
              <span class="detail-msg">{esc(c["message"] or "(no message)")}</span>
            </div>
            <pre>{esc((c["detail"] or "").strip())}</pre>
          </div>
            '''
        return f'''
        <div class="row{row_extra_cls}" tabindex="0" aria-expanded="false" data-suite="{esc(c['suite'])}" data-classname="{esc(c['classname'])}" data-name="{esc(c['name'])}">
          <div class="status {pill_class}">{label_of(status)}</div>
          <div class="nowrap" title="{esc(c['name'])}">{esc(c['name'])}</div>
          <div class="nowrap" title="{esc(c['classname'])}">{esc(c['classname'])}</div>
          <div class="nowrap" title="{esc(c['suite'])}">{esc(c['suite'])}</div>
          <div>{seconds_fmt(float(c['time']))}</div>
          {details_html}
        </div>
        '''

    # Build main content structure with collapsible sections
    def build_main_content():
        ...
        for suite in sorted(structure.keys()):
            ...
            main_html += f'''
        <div class="suite-section" data-suite="{esc(suite)}">
          <div class="suite-header" data-type="suite" data-suite="{esc(suite)}">
            <div class="suite-title">
              <span class="collapse-icon">▼</span>
              <span class="item-icon">📁</span>
              <span class="item-text">{esc(suite)}</span>
            </div>
...
'''
            ...
            for classname in sorted(structure[suite].keys()):
                ...
                main_html += f'''
            <div class="class-section" data-suite="{esc(suite)}" data-classname="{esc(classname)}">
              <div class="class-header" data-type="class" data-suite="{esc(suite)}" data-classname="{esc(classname)}">
                <div class="class-title">
                  <span class="collapse-icon">▼</span>
                  <span class="item-icon">📦</span>
                  <span class="item-text">{esc(classname)}</span>
                </div>
...
'''
    ...
    def build_sidebar():
        ...
        sidebar_html += f'<input type="search" placeholder="Search {esc(title)}..." />\n'
        sidebar_html += '</div>\n'
        sidebar_html += '<div class="sidebar-content">\n'
        
        for suite in sorted(structure.keys()):
            sidebar_html += f'<div class="sidebar-item suite-item" data-suite="{esc(suite)}">\n'
            sidebar_html += f'  <div class="sidebar-label" data-type="suite" data-suite="{esc(suite)}">\n'
            sidebar_html += f'    <span class="collapse-icon">▼</span>\n'
            sidebar_html += f'    <span class="item-icon">📁</span>\n'
            sidebar_html += f'    <span class="item-text">{esc(suite)}</span>\n'
            sidebar_html += f'  </div>\n'
            sidebar_html += f'  <div class="sidebar-children suite-children">\n'
            
            for classname in sorted(structure[suite].keys()):
                sidebar_html += f'    <div class="sidebar-item class-item" data-suite="{esc(suite)}" data-classname="{esc(classname)}">\n'
                sidebar_html += f'      <div class="sidebar-label" data-type="class" data-suite="{esc(suite)}" data-classname="{esc(classname)}">\n'
                sidebar_html += f'        <span class="collapse-icon">▼</span>\n'
                sidebar_html += f'        <span class="item-icon">📦</span>\n'
                sidebar_html += f'        <span class="item-text">{esc(classname)}</span>\n'
                sidebar_html += f'      </div>\n'
                sidebar_html += f'      <div class="sidebar-children class-children">\n'
                
                for name in sorted(structure[suite][classname]):
                    sidebar_html += f'        <div class="sidebar-item case-item" data-suite="{esc(suite)}" data-classname="{esc(classname)}" data-name="{esc(name)}">\n'
                    sidebar_html += f'          <div class="sidebar-label" data-type="case" data-suite="{esc(suite)}" data-classname="{esc(classname)}" data-name="{esc(name)}">\n'
                    sidebar_html += f'            <span class="item-icon">🧪</span>\n'
                    sidebar_html += f'            <span class="item-text">{esc(name)}</span>\n'
                    sidebar_html += f'          </div>\n'
                    sidebar_html += f'        </div>\n'
    ...
    main_content_html = build_main_content()
...
<body>
  ...
  <div class="main-content">
    <h1>{esc(title)}</h1>
    <div class="muted small">Generate Time : {esc(now)}</div>
...
def main():
    ...
    print(f"Suites: {data['totals']['suites']}, Tests: {data['totals']['tests']}, "
          f"Passed: {data['totals']['tests'] - data['totals']['failures'] - data['totals']['errors'] - data['totals']['skipped']}, "
          f"Failures: {data['totals']['failures']}, Errors: {data['totals']['errors']}, Skipped: {data['totals']['skipped']}")
